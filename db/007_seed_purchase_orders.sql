/* ============================================================================
   ...
   Phase 14 (db/036) — VendorCode + VendorName moved from PurchaseOrders to
   PurchaseOrderLines. Each line in this seed now carries its own vendor;
   within a single PO every line shares the same vendor (the historical seed
   shape), so post-Phase-14 list/detail vendor collapse still returns the
   familiar single value via the MIN=MAX summary in the repo. Mixed-vendor
   cases are exercised by smoke-phase-14-vendor-at-line.
   ============================================================================ */
...
-- PO-2312-091 — historical closed (demo of closed-PO appearance). Vendor: V-ACME.
IF NOT EXISTS (SELECT 1 FROM dbo.PurchaseOrders WHERE PoNumber = 'PO-2312-091')
BEGIN
    INSERT INTO dbo.PurchaseOrders (Id, PoNumber, WarehouseId, OrderDate, ExpectedDate, Status, Notes, CreatedBy, CreatedAt, ClosedAt)
...
-- PO-2401-018 — primary backfill target. Every PL-2847 positive lands on these lines. Vendor: V-ACME.
...
BEGIN
    INSERT INTO dbo.PurchaseOrderLines (Id, PurchaseOrderId, LineNumber, ItemCode, Description, OrderedQty, ReceivedQty, VendorCode, VendorName)
    VALUES ('77777777-7777-7777-7777-020100000001', '66666666-6666-6666-6666-000000000002', 1, 'PCBA-AX450-R2', N'PCBA AX450 Rev 2 - 1.5GHz',          5500, 0, 'V-ACME', N'Acme Components Ltd');
END
GO
IF NOT EXISTS (SELECT 1 FROM dbo.PurchaseOrderLines WHERE Id = '77777777-7777-7777-7777-020100000002')
BEGIN
    INSERT INTO dbo.PurchaseOrderLines (Id, PurchaseOrderId, LineNumber, ItemCode, Description, OrderedQty, ReceivedQty, VendorCode, VendorName)
    VALUES ('77777777-7777-7777-7777-020100000002', '66666666-6666-6666-6666-000000000002', 2, 'PCBA-AX451-R2', N'PCBA AX451 Rev 2 - 1.8GHz',          2000, 0, 'V-ACME', N'Acme Components Ltd');
END
GO
IF NOT EXISTS (SELECT 1 FROM dbo.PurchaseOrderLines WHERE Id = '77777777-7777-7777-7777-020100000003')
BEGIN
    INSERT INTO dbo.PurchaseOrderLines (Id, PurchaseOrderId, LineNumber, ItemCode, Description, OrderedQty, ReceivedQty, VendorCode, VendorName)
    VALUES ('77777777-7777-7777-7777-020100000003', '66666666-6666-6666-6666-000000000002', 3, 'CAP-470UF-25V', N'Capacitor 470µF 25V',                7000, 0, 'V-ACME', N'Acme Components Ltd');
END
GO
IF NOT EXISTS (SELECT 1 FROM dbo.PurchaseOrderLines WHERE Id = '77777777-7777-7777-7777-020100000004')
BEGIN
    INSERT INTO dbo.PurchaseOrderLines (Id, PurchaseOrderId, LineNumber, ItemCode, Description, OrderedQty, ReceivedQty, VendorCode, VendorName)
    VALUES ('77777777-7777-7777-7777-020100000004', '66666666-6666-6666-6666-000000000002', 4, 'RES-10K-1%',    N'Resistor 10kΩ 1% 0805',            20000, 0, 'V-ACME', N'Acme Components Ltd');
END
GO
IF NOT EXISTS (SELECT 1 FROM dbo.PurchaseOrderLines WHERE Id = '77777777-7777-7777-7777-020100000005')
BEGIN
    INSERT INTO dbo.PurchaseOrderLines (Id, PurchaseOrderId, LineNumber, ItemCode, Description, OrderedQty, ReceivedQty, VendorCode, VendorName)
    VALUES ('77777777-7777-7777-7777-020100000005', '66666666-6666-6666-6666-000000000002', 5, 'CONN-USB-C-16', N'USB-C 16-pin connector',              500, 0, 'V-ACME', N'Acme Components Ltd');
END
GO
IF NOT EXISTS (SELECT 1 FROM dbo.PurchaseOrderLines WHERE Id = '77777777-7777-7777-7777-020100000006')
BEGIN
    INSERT INTO dbo.PurchaseOrderLines (Id, PurchaseOrderId, LineNumber, ItemCode, Description, OrderedQty, ReceivedQty, VendorCode, VendorName)
    VALUES ('77777777-7777-7777-7777-020100000006', '66666666-6666-6666-6666-000000000002', 6, 'LCD-3.5-IPS',   N'LCD 3.5" IPS',                         100, 0, 'V-ACME', N'Acme Components Ltd');
END
GO
IF NOT EXISTS (SELECT 1 FROM dbo.PurchaseOrderLines WHERE Id = '77777777-7777-7777-7777-020100000007')
BEGIN
    INSERT INTO dbo.PurchaseOrderLines (Id, PurchaseOrderId, LineNumber, ItemCode, Description, OrderedQty, ReceivedQty, VendorCode, VendorName)
    VALUES ('77777777-7777-7777-7777-020100000007', '66666666-6666-6666-6666-000000000002', 7, 'SHIELD-RF-A1',  N'RF shield A1',                         500, 0, 'V-ACME', N'Acme Components Ltd');
END
GO

-- PO-2401-019 — second-oldest WH-01 PO; future FIFO split target for RES-10K-1% + extra PCBA capacity. Vendor: V-NEXTRON.
...
BEGIN
    INSERT INTO dbo.PurchaseOrderLines (Id, PurchaseOrderId, LineNumber, ItemCode, Description, OrderedQty, ReceivedQty, VendorCode, VendorName)
    VALUES ('77777777-7777-7777-7777-030100000001', '66666666-6666-6666-6666-000000000003', 1, 'RES-10K-1%',    N'Resistor 10kΩ 1% 0805', 5000, 0, 'V-NEXTRON', N'Nextron Electronics');
END
GO
IF NOT EXISTS (SELECT 1 FROM dbo.PurchaseOrderLines WHERE Id = '77777777-7777-7777-7777-030100000002')
BEGIN
    INSERT INTO dbo.PurchaseOrderLines (Id, PurchaseOrderId, LineNumber, ItemCode, Description, OrderedQty, ReceivedQty, VendorCode, VendorName)
    VALUES ('77777777-7777-7777-7777-030100000002', '66666666-6666-6666-6666-000000000003', 2, 'PCBA-AX450-R2', N'PCBA AX450 Rev 2',     2000, 0, 'V-NEXTRON', N'Nextron Electronics');
END
GO

-- PO-2403-044 — newest WH-01 PO; third FIFO target for RES-10K-1%. Vendor: V-FORTIS.
...
BEGIN
    INSERT INTO dbo.PurchaseOrderLines (Id, PurchaseOrderId, LineNumber, ItemCode, Description, OrderedQty, ReceivedQty, VendorCode, VendorName)
    VALUES ('77777777-7777-7777-7777-040100000001', '66666666-6666-6666-6666-000000000004', 1, 'RES-10K-1%',    N'Resistor 10kΩ 1% 0805', 3000, 0, 'V-FORTIS', N'Fortis Microparts');
END
GO
IF NOT EXISTS (SELECT 1 FROM dbo.PurchaseOrderLines WHERE Id = '77777777-7777-7777-7777-040100000002')
BEGIN
    INSERT INTO dbo.PurchaseOrderLines (Id, PurchaseOrderId, LineNumber, ItemCode, Description, OrderedQty, ReceivedQty, VendorCode, VendorName)
    VALUES ('77777777-7777-7777-7777-040100000002', '66666666-6666-6666-6666-000000000004', 2, 'CAP-470UF-25V', N'Capacitor 470µF 25V',   2000, 0, 'V-FORTIS', N'Fortis Microparts');
END
GO
...
BEGIN
    INSERT INTO dbo.PurchaseOrderLines (Id, PurchaseOrderId, LineNumber, ItemCode, Description, OrderedQty, ReceivedQty, VendorCode, VendorName)
    VALUES ('77777777-7777-7777-7777-050100000001', '66666666-6666-6666-6666-000000000005', 1, 'PCBA-AX450-R2', N'PCBA AX450 Rev 2', 1000, 0, 'V-ACME', N'Acme Components Ltd');
END
GO

-- PO-2402-023 — Vendor: V-NEXTRON.
...
BEGIN
    INSERT INTO dbo.PurchaseOrderLines (Id, PurchaseOrderId, LineNumber, ItemCode, Description, OrderedQty, ReceivedQty, VendorCode, VendorName)
    VALUES ('77777777-7777-7777-7777-060100000001', '66666666-6666-6666-6666-000000000006', 1, 'CAP-470UF-25V', N'Capacitor 470µF 25V', 2000, 0, 'V-NEXTRON', N'Nextron Electronics');
END
GO
IF NOT EXISTS (SELECT 1 FROM dbo.PurchaseOrderLines WHERE Id = '77777777-7777-7777-7777-060100000002')
BEGIN
    INSERT INTO dbo.PurchaseOrderLines (Id, PurchaseOrderId, LineNumber, ItemCode, Description, OrderedQty, ReceivedQty, VendorCode, VendorName)
    VALUES ('77777777-7777-7777-7777-060100000002', '66666666-6666-6666-6666-000000000006', 2, 'PCBA-AX450-R2', N'PCBA AX450 Rev 2',    1000, 0, 'V-NEXTRON', N'Nextron Electronics');
END
GO

-- PO-2403-051 — Vendor: V-FORTIS.
...
BEGIN
    INSERT INTO dbo.PurchaseOrderLines (Id, PurchaseOrderId, LineNumber, ItemCode, Description, OrderedQty, ReceivedQty, VendorCode, VendorName)
    VALUES ('77777777-7777-7777-7777-070100000001', '66666666-6666-6666-6666-000000000007', 1, 'RES-10K-1%', N'Resistor 10kΩ 1% 0805', 3000, 0, 'V-FORTIS', N'Fortis Microparts');
END
GO
...
BEGIN
    INSERT INTO dbo.PurchaseOrderLines (Id, PurchaseOrderId, LineNumber, ItemCode, Description, OrderedQty, ReceivedQty, VendorCode, VendorName)
    VALUES ('77777777-7777-7777-7777-080100000001', '66666666-6666-6666-6666-000000000008', 1, 'PCBA-AX450-R2', N'PCBA AX450 Rev 2', 2000, 0, 'V-ACME', N'Acme Components Ltd');
END
GO

-- PO-2402-032 — Vendor: V-NEXTRON.
...
BEGIN
    INSERT INTO dbo.PurchaseOrderLines (Id, PurchaseOrderId, LineNumber, ItemCode, Description, OrderedQty, ReceivedQty, VendorCode, VendorName)
    VALUES ('77777777-7777-7777-7777-090100000001', '66666666-6666-6666-6666-000000000009', 1, 'RES-10K-1%',    N'Resistor 10kΩ 1% 0805', 5000, 0, 'V-NEXTRON', N'Nextron Electronics');
END
GO
IF NOT EXISTS (SELECT 1 FROM dbo.PurchaseOrderLines WHERE Id = '77777777-7777-7777-7777-090100000002')
BEGIN
    INSERT INTO dbo.PurchaseOrderLines (Id, PurchaseOrderId, LineNumber, ItemCode, Description, OrderedQty, ReceivedQty, VendorCode, VendorName)
    VALUES ('77777777-7777-7777-7777-090100000002', '66666666-6666-6666-6666-000000000009', 2, 'CAP-470UF-25V', N'Capacitor 470µF 25V',   1500, 0, 'V-NEXTRON', N'Nextron Electronics');
END
GO

-- PO-2403-061 — Vendor: V-FORTIS.
...
BEGIN
    INSERT INTO dbo.PurchaseOrderLines (Id, PurchaseOrderId, LineNumber, ItemCode, Description, OrderedQty, ReceivedQty, VendorCode, VendorName)
    VALUES ('77777777-7777-7777-7777-100100000001', '66666666-6666-6666-6666-000000000010', 1, 'PCBA-AX451-R2', N'PCBA AX451 Rev 2', 1500, 0, 'V-FORTIS', N'Fortis Microparts');
END
GO
...
BEGIN
    INSERT INTO dbo.PurchaseOrderLines (Id, PurchaseOrderId, LineNumber, ItemCode, Description, OrderedQty, ReceivedQty, VendorCode, VendorName)
    VALUES ('77777777-7777-7777-7777-110100000001', '66666666-6666-6666-6666-000000000011', 1, 'PCBA-AX450-R2', N'PCBA AX450 Rev 2', 1000, 0, 'V-ACME', N'Acme Components Ltd');
END
GO

PRINT '007_seed_purchase_orders.sql complete — 11 POs, 22 PO lines (Phase 14: vendor at line grain).';
GO
