...
async function exportToExcel() {
  return queueExport('/api/exports/transactions', 'Export');
}

/* The KTF form export posts the identical filter to a different endpoint —
   the server pins Kind='receive' and renders the form layout, so nothing
   about the filter needs to change on this side. */
async function exportKtf() {
  return queueExport('/api/exports/ktf', 'KTF export');
}

async function queueExport(endpoint, label) {
  if (currentTotal === 0) {
  ...
  try {
    ...
    if (!resp.ok) {
      ...
      try { const j = await resp.json(); if (j?.title) title = j.title; } catch {}
      showToast(`${label} not queued`, title, 'danger');
      return;
    }
    const body = await resp.json();
    showToast(`${label} queued`, body.message || `Check ${body.email}`);
  } catch (e) {
    console.error('export queue failed', e);
    showToast('Network error', `Could not queue ${label.toLowerCase()}`, 'danger');
  }
}
...
document.getElementById('btn-export-ktf').addEventListener('click', exportKtf);
