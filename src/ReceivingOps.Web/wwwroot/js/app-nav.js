 ...
 * ----------------------------------------------------------------------------
 * Drop-in replacement for window.confirm(). Self-bootstraps CSS into <head>
 * on script load; markup is built per-call and removed after the promise
 * resolves so the DOM stays clean.
 *
 * Returns Promise<boolean> — resolves true on Confirm click, false on Cancel
 * / Escape / backdrop click. Enter does NOT auto-commit — safer default
 * against accidental destructive actions.
 *
 * Usage:
 *   const ok = await confirmAction({
 *     title: 'Delete item WIDGET-1000?',
 *     message: 'Windows will cascade.',
 *     icon: 'trash',           // 'trash' | 'warning' | 'info'
 *     confirmLabel: 'Delete',
 *     cancelLabel: 'Cancel',   // optional, defaults to 'Cancel'
 *     danger: true,            // red confirm button
 *     details: { 'Pull #': 'PL-2847', 'Windows': '3' }  // optional rows
 *   });
 *   if (!ok) return;
 * ========================================================================== */
(function () {
  ...
  // Inject CSS once, on script load. Tokens reference the same --surface /
  // --border / --text-muted variables the rest of the app uses so the modal
  // tracks theme automatically; semantic colors (red/amber/blue) are hard-
  // coded per the design spec so danger/warning/info read the same across
  // light/midnight/slate.
  const css = `
  ...
  // Icon class + tint mapping. Tint defaults: danger flag overrides icon to
  // red regardless of which icon you picked; otherwise icon name picks the
  // tint (so 'info' → blue, 'warning'/'trash' → amber).
  const ICONS = {
    trash:   'bi-trash',
  ...
  window.confirmAction = function (opts) {
    return new Promise((resolve) => {
      ...
      // (a delete button on a row, etc.). Safer default than blackholing focus.
      const prevFocus = document.activeElement;
      // Initial focus on Cancel — destructive-action safety per design spec.
      cancelBtn.focus();
      ...
      function onKey(e) {
        ...
        // Enter is intentionally NOT bound — operator must click Confirm to
        // commit a destructive action (per design spec).
      }
      document.addEventListener('keydown', onKey, true);

      backdrop.addEventListener('click', (e) => {
        if (e.target === backdrop) cleanup(false);  // backdrop click → cancel
      });
